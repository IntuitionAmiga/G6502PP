...
The snippet below shows an opcode handler for `ADD #<immediate>` which is nice and simple.

- im() expands code that calculates the location of the immediate value, which it stores in a local `iAddress`.
- load() expands to code that loads the byte at the location into the named parameter, a local `iData`. 
- The logical AND is just a vanilla imperative statement.
- updateNZ() expands to code that updates the status register flags based on the named parameter.
- size() expands to code that increments the program counter.
- dispatch() expands to code that concludes the curent handler.
...
The macro expansion, commended:

```go
	case AND_IM:
		{
			{ // Block generared by macro load()
				{ // Block generated by im()
					iAddress = iProgramCounter + 1 // The location of the immediate
				}
				iData = (*(*Byte)(unsafe.Pointer(uintptr(pOutside) + uintptr(iAddress))))
			}
			iAccumulator &= iData
			{ // Block generated by updateNZ()
				iStatus &= F_CLR_NZ
				if (iAccumulator) == 0 {
					iStatus |= F_ZERO
				} else {
					iStatus |= (iAccumulator & F_NEGATIVE)
				}
			}
			iProgramCounter += SIZE_AND_IM // Generated by size()
			continue // Generated by dispatch()
		}

```

The interpreter is using locally pinned values for `iProgramCounter`, `iAccumulator` and `iStatus` as these are hot values, updated on amost every instruction.

- Memory access is via an unsafe pointer to skip bounds checking. This is "safe" because the memory is guaranteed to be a 65536-byte block already and the emulated program counter is a uint16 thus incapable of representing values that are out of bounds.
- Macro expansions only get more complicated for other addressing modes and more involved instructions.
